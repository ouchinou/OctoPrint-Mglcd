...
class Nextion(object):
    ...
    def nxWrite(self,s):
        ...
        self.ser.write(s.encode('latin-1') if isinstance(s, str) else s)
    ...
    def nxRead(self,cmax=0,timeout=0.5):
        ...
        def _reader():
            ...
            while timeout==0 or (time.perf_counter()-time_now)<timeout:
                if self.debug:
...
class NextionPlugin(octoprint.plugin.StartupPlugin,
    ...
    def parseLog(self):
        ...
        if b'\x00' in self.receiveLog:
            self.receiveLog.popleft()
        if b'\xff' in self.receiveLog:
